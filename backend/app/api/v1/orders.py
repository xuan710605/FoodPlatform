...
from app.schemas.order import OrderCreate,OrderPage,OrderSummary,PaymentRequest
router=APIRouter()
...
def list_orders(request:Request,page:int=Query(1,ge=1),page_size:int=Query(20,ge=1,le=100),user:UserIdentity=Depends(get_current_user)):return {"success":True,"data":request.app.state.order_service.list(user.id,page,page_size),"message":"ok","request_id":request.state.request_id}
