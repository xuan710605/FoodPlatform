...
export function ConsumerLayout() {
  const { cart, favorites, loggedIn, currentUser } = useApp(); const [mobile, setMobile] = useState(false); const [query, setQuery] = useState(''); const navigate = useNavigate()
  const submit = (e: React.FormEvent) => { e.preventDefault(); navigate(`/products?q=${encodeURIComponent(query)}`); setMobile(false) }
  const workspacePath=currentUser?.roles.includes('ADMIN')?'/admin/dashboard':currentUser?.roles.includes('MERCHANT')?'/merchant/dashboard':null
