...
interface AppState {
  ...
  updateCart: (id: number, patch: Partial<CartLine>) => void
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const notify = (message: string, tone: Toast['tone'] = 'success') => {
    ...
    window.setTimeout(() => setToasts((v) => v.filter((x) => x.id !== id)), 2300)
  }
  useEffect(() => {
    const labels = ['订单详情', '去支付', '评价', '查看', '导出订单', '历史版本', '选择图片', '手动修改', '处理记录']
    const feedback = (event: MouseEvent) => {
      const button = (event.target as HTMLElement).closest('button')
      if (button && labels.some((label) => button.textContent?.trim().includes(label))) notify('演示操作已响应', 'info')
    }
    document.addEventListener('click', feedback)
    return () => document.removeEventListener('click', feedback)
  }, [])
  const addCart = (productId: number, quantity = 1) => {
    setCart((lines) => {
      const old = lines.find((x) => x.productId === productId)
      return old ? lines.map((x) => x.productId === productId ? { ...x, quantity: x.quantity + quantity } : x) : [...lines, { productId, quantity, selected: true }]
    })
    notify('已加入购物车')
  }
  const updateCart = (id: number, patch: Partial<CartLine>) => setCart((v) => v.map((x) => x.productId === id ? { ...x, ...patch } : x))
  const removeCart = (id: number) => { setCart((v) => v.filter((x) => x.productId !== id)); notify('商品已移除', 'info') }
  const toggleFavorite = (id: number) => { setFavorites((v) => v.includes(id) ? v.filter((x) => x !== id) : [...v, id]); notify(favorites.includes(id) ? '已取消收藏' : '已加入收藏', 'info') }
  const toggleCompare = (id: number) => {
    if (!compare.includes(id) && compare.length >= 4) return notify('最多同时对比 4 件商品', 'error')
    setCompare((v) => v.includes(id) ? v.filter((x) => x !== id) : [...v, id])
