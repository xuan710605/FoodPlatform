...
interface AppState{cart:ApiCartItem[];favorites:number[];browsingHistory:BrowsingHistoryEntry[];filterHistory:FilterHistoryEntry[];compare:number[];loggedIn:boolean;currentUser:CurrentUser|null;userLoading:boolean;toasts:Toast[];refreshUser:()=>Promise<void>;refreshCart:()=>Promise<void>;addCart:(id:number,quantity?:number)=>Promise<void>;updateCart:(itemId:number,quantity:number)=>Promise<void>;removeCart:(itemId:number)=>Promise<void>;toggleFavorite:(id:number)=>void;trackProductView:(product:Product)=>void;addFilterHistory:(entry:Omit<FilterHistoryEntry,'id'|'time'>)=>void;replaceFilterHistory:(items:FilterHistoryEntry[])=>void;toggleCompare:(id:number)=>void;login:(user:CurrentUser)=>void;logout:()=>void;notify:(message:string,tone?:Toast['tone'])=>void}
const Context=createContext<AppState|null>(null)
const favoritesKey=(userId:number)=>`foodplatform_favorites_${userId}`
const readFavorites=(userId:number)=>{try{return JSON.parse(localStorage.getItem(favoritesKey(userId))||'[]') as number[]}catch{return []}}
const requireLogin=()=>{window.location.assign('/login')}
export function AppProvider({children}:{children:ReactNode}){
 const hasToken=Boolean(localStorage.getItem('access_token'));const [cart,setCart]=useState<ApiCartItem[]>([]);const [favorites,setFavorites]=useState<number[]>([]);const [browsingHistory,setBrowsingHistory]=useState<BrowsingHistoryEntry[]>([]);const [filterHistory,setFilterHistory]=useState<FilterHistoryEntry[]>([]);const [compare,setCompare]=useState<number[]>([1,2,16]);const [loggedIn,setLoggedIn]=useState(hasToken);const [currentUser,setCurrentUser]=useState<CurrentUser|null>(null);const [userLoading,setUserLoading]=useState(hasToken);const [toasts,setToasts]=useState<Toast[]>([])
 ...
 const clearUserMemory=()=>{setCurrentUser(null);setCart([]);setFavorites([]);setBrowsingHistory([]);setFilterHistory([]);setLoggedIn(false)}
 const loadUserLocalData=(userId:number)=>{setFavorites(readFavorites(userId));setBrowsingHistory(getBrowseHistory(userId));setFilterHistory(getFilterHistory(userId))}
 const refreshUser=async()=>{if(!localStorage.getItem('access_token')){clearUserMemory();setUserLoading(false);return}setUserLoading(true);try{const user=await getCurrentUser();setCurrentUser(user);loadUserLocalData(user.id);setLoggedIn(true);localStorage.setItem('current_user',JSON.stringify(user));await refreshCart()}catch{clearUserMemory();localStorage.removeItem('current_user')}finally{setUserLoading(false)}}
 useEffect(()=>{if(hasToken)void refreshUser()},[])
 useEffect(()=>{if(currentUser)localStorage.setItem(favoritesKey(currentUser.id),JSON.stringify(favorites))},[favorites,currentUser])
 ...
 const toggleFavorite=(id:number)=>{if(!currentUser)return requireLogin();setFavorites(v=>v.includes(id)?v.filter(x=>x!==id):[...v,id]);notify(favorites.includes(id)?'已取消收藏':'已加入收藏','info')}
 ...
 const login=(user:CurrentUser)=>{setCurrentUser(user);loadUserLocalData(user.id);setLoggedIn(true);setUserLoading(false);localStorage.setItem('current_user',JSON.stringify(user));void refreshCart();notify('登录成功，欢迎回来')}
 ...
 const value=useMemo(()=>({cart,favorites,browsingHistory,filterHistory,compare,loggedIn,currentUser,userLoading,toasts,refreshUser,refreshCart,addCart,updateCart,removeCart,toggleFavorite,trackProductView,addFilterHistory,replaceFilterHistory,toggleCompare,login,logout,notify}),[cart,favorites,browsingHistory,filterHistory,compare,loggedIn,currentUser,userLoading,toasts])
