...
export function ProductCard({ product, compact = false }: { product: Product; compact?: boolean }) {
  ...
  return <article className={`product-card ${compact ? 'compact' : ''}`}>
    <div className="product-image-wrap">
      <Link to={`/product/${productCodeOf(product)}`}><SafeImage src={product.image} alt={product.name}/></Link>
    ...
    <div className="product-card-body">
      ...
      <Link to={`/product/${productCodeOf(product)}`} className="product-name">{product.name}</Link>
